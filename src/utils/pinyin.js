...
function pySegSort(tarr) {
  for (let i = 0; i < tarr.length; i++) {
    let inputV = tarr[i].name // ---- 排序指定的属性
    let str = ConvertPinyin(inputV)
    tarr[i].nickName = str // 转拼音后得到的值赋值到一个新的属性中
  }
  ...
  if (res.segs[0] && res.segs[0].letter == '*') {
    let first = res.segs[0]
    first.letter = '#'
    res.segs.shift()
    res.segs.push(first)
  }
